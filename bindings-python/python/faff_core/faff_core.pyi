...
class PlanManager:
    ...
    def get_session_hints(self, date: datetime.date) -> List[Dict]:
        """
        Get session hints from plans valid for a given date.

        Hints are generated by plugins (e.g. one per POC/Support tracker) and
        drive title suggestions and field pre-weighting at session start.

        Returns:
            List of dicts with keys:
                title (str): Hint title (e.g. tracker name)
                role (Optional[str]): Suggested role (source-prefixed)
                subject (Optional[str]): Suggested subject (source-prefixed)
                impact (Optional[str]): Suggested impact (source-prefixed)
                mode (Optional[str]): Suggested mode (source-prefixed)
                trackers (list[str]): Associated tracker IDs
        """
        ...

    def get_tracker_mappings(self, date: datetime.date) -> List[Dict]:
        """
        Get tracker mappings derived from vocabulary mapping rules for a given date.

        Each entry maps specific session field values back to a tracker, enabling
        auto-derivation of trackers from session fields at start time.

        Returns:
            List of dicts with keys:
                tracker_id (str): Prefixed tracker ID (e.g., "element:1231232")
                tracker_name (str): Human-readable tracker name
                role (Optional[str]): Required role value, or None if unconstrained
                subject (Optional[str]): Required subject value, or None if unconstrained
                impact (Optional[str]): Required impact value, or None if unconstrained
                mode (Optional[str]): Required mode value, or None if unconstrained
        """
        ...
