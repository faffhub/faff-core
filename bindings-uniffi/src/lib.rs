...
//!
//! Surface: read the daily log, start/stop sessions, edit a session's
//! semantic fields or times (matched by id-prefix), vocabulary queries, and
//! a `recent_prototypes` convenience that replaces the old plan-level
//! "intent" concept with history-derived session shapes.
...
pub enum FaffError {
    ...
    InvalidDate(String),
    #[error("invalid time '{0}': expected an RFC3339 timestamp")]
    InvalidTime(String),
...
impl FaffWorkspace {
    ...
    /// Stop all currently active sessions in today's log at "now". Maps the
    /// core's "no active session" error to the typed `NoActiveSession`
    /// variant on a best-effort basis.
    pub fn stop_current_session(&self) -> Result<(), FaffError> {
        self.rt
            .block_on(self.inner.logs().stop_all_active_sessions())
            .map_err(|e| {
                let msg = e.to_string();
    ...
    /// Replace the semantic fields of a session identified by date +
    /// id_prefix. Mirrors `LogManager::update_session`.
    #[allow(clippy::too_many_arguments)]
    pub fn update_session(
        ...
        let parsed = parse_date(&date)?;
        self.rt
            .block_on(self.inner.logs().update_session(
                parsed, &id_prefix, title, role, impact, mode, subject, trackers, note,
            ))
            .map_err(|e| FaffError::Other(e.to_string()))
    }

    /// Reschedule the start/end times of a session identified by date +
    /// id_prefix. Mirrors `LogManager::reschedule_session`. Times arrive as
    /// RFC3339 strings and are interpreted in the workspace timezone so the
    /// stored wall-clock time round-trips faithfully.
    pub fn reschedule_session(
        ...
        let start_dt = parse_time(&start, tz)?;
        let end_dt = match end {
            Some(s) => Some(parse_time(&s, tz)?),
            None => None,
        };
        self.rt
            .block_on(
                self.inner
                    .logs()
                    .reschedule_session(parsed, &id_prefix, start_dt, end_dt),
            )
            .map_err(|e| FaffError::Other(e.to_string()))
    }
...
/// Parse an RFC3339 timestamp and reinterpret it in the workspace timezone,
/// preserving the instant. Keeping the named tz (rather than a raw offset)
/// means the value serializes back with the correct local wall-clock time.
fn parse_time(s: &str, tz: Tz) -> Result<DateTime<Tz>, FaffError> {
    DateTime::parse_from_rfc3339(s)
        .map(|dt| dt.with_timezone(&tz))
        .map_err(|_| FaffError::InvalidTime(s.to_string()))
}
